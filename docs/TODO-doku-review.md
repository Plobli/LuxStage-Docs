...
- [ ] **Tab „Setup"** — die Doku-Seite heißt `setup.md`, die App-Sidebar zeigt **„Setup"** (`tab.gassenturm`), intern „Gassenturm". Passt. Aber: `tab.buehne` = „Beleuchtungsgestelle" und `tab.towers` = „Beleuchtungsgestelle" doppelt, `tab.obermaschinerie` = „Obermaschinerie" taucht in `setup.md` **gar nicht** auf — dort heißt alles „Zugstangen" (`tab.bars`). Nutzer sieht „Obermaschinerie" in der Sidebar und findet nichts.
