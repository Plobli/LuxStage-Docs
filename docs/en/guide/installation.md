...
Automated installation on Linux or Raspberry Pi. A script takes care of everything — Docker, reverse proxy, database.
...
**Admin password** — At least 8 characters. You will log in as `admin` with this:
```
Admin password: ••••••••
```
...
::: tip Technician password
The script automatically generates a separate password for the `tech` account at the end. Write it down!
:::

## Step 4 — What gets set up

- **LuxStage server** — starts automatically on system boot
...
|---------|----------|----------|
| Admin | `admin` | Password from the installation |
| Technician | `tech` | Auto-generated by the installer |

### User roles

| Username | Role | Permissions |
|----------|------|-------------|
| `admin` | Administrator | Everything, incl. templates, backup, updates |
| `tech` | Technician | Read and edit productions, no templates/backup |

The tech password is displayed at the end of the installation.
