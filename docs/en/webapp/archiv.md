# Archive

The **Archive** contains all shows that have been completed and archived. Archived shows no longer appear in the normal shows overview.

## Access the archive

Click the **archive icon** (📦) in the left sidebar.

If no shows have been archived, the message **"No archived shows"** is displayed.

## Archive a show

In the **Shows overview**, hover over a show row and click the archive icon on the right. The show is immediately moved to the archive.

## View archived shows

The archive displays all archived shows as a list, with name, date, season, and last edited by – the same columns as in the normal shows overview, **but without sorting and without search**.

Hovering over a row reveals two buttons on the right:

- **Restore** – moves the show back to the normal shows overview
- **Delete** – permanently deletes the show after confirmation, including all data

::: warning Note
Permanent deletion cannot be undone.
:::
